...
attr_accessor :report_name,
  ...
  :publisher_order,         :publisher_filter,           :publisher_exact,          :publisher_sort,
  :disposition_order,       :disposition_filter,         :disposition_exact,        :disposition_sort,
  :publication_date_order,  :publication_date_filter,    :publication_date_exact,   :publication_date_sort,
  :format_order,            :format_filter,              :format_exact,             :format_sort,
  ...
  def initialize(
    ...
    publisher_order=nil,         publisher_filter=nil,           publisher_exact=nil,          publisher_sort=nil,
    disposition_order=nil,       disposition_filter=nil,         disposition_exact=nil,        disposition_sort=nil,
    publication_date_order=nil,  publication_date_filter=nil,    publication_date_exact=nil,   publication_date_sort=nil,
...
##############################################################################
  ...
  def field_summary(summary_count, summary_field, summary_field_value, summary_cost='', summary_value='', summary_genre='',report_format=nil, colspan=20)
    ...
    summary_row = %Q~<tr style="background-color:#dddddd;font-size:14pt;padding:8px;font-decoration:bold;"><td colspan=#{colspan}> #{summary_count} items in #{summary_field} #{summary_field_value} | cost: $ #{summary_cost} | sell value: $ #{summary_value}</td></tr>~
  else
    summary_row = %Q~</table>
...
class PeekItem < WEBrick::HTTPServlet::AbstractServlet
  ...
  edition      = $collection.items[key].edition
  authors      = $collection.items[key].authors
  publisher 	= $collection.items[key].publisher
  format       = $collection.items[key].format
  disposition  = $collection.items[key].disposition
  edition 	= $collection.items[key].edition
  isbn 	= $collection.items[key].isbn
...
<div class="item">
  ...
  <br style="clear:both;" />
        ...
        <div class="section">&#8227; id: #{id}</div>
          <div class="label" style="width:25%;"><span class='identifier'>author(s)</span><br />#{authors}</div>
          <div class="label" style="width:25%;"><span class='identifier'>format</span><br />#{format}</div>
          <div class="label" style="width:25%;"><span class='identifier'>pages</span><br />#{pages}</div>
          <div class="label" style="width:25%;"><span class='identifier'>location</span><br />#{location}</div>
          <br style="clear:both;" />
          <div class="label" style="width:25%;"><span class='identifier'>isbn</span><br />#{isbn}</div>
          <div class="label" style="width:25%;"><span class='identifier'>edition</span><br />#{edition}</div>
          <div class="label" style="width:25%;"><span class='identifier'>publisher</span><br />#{publisher}</div>
          <br style="clear:both;" />
          <div class="label" style="width:25%;"><span class='identifier'>publication date</span><br />#{publication_date}</div>
          <div class="label" style="width:25%;"><span class='identifier'>language</span><br />#{language}</div>
          <div class="label" style="width:25%;"><span class='identifier'>disposition</span><br />#{disposition}</div>
          <br style="clear:both;" />
          <div class="label" style="width:25%;"><span class='identifier'>cost</span><br />#{cost}</div>
          ...
          <div class="label" style="width:50%;"><span class='identifier'>tags</span><br />#{tags}</div>
          <br style="clear:both;" />
          <div class="section">&#8227; links / notes</div>
          <div class="label" style="width:100%;"><span class='identifier'>notes</span><br />#{notes}</div>
          <br style="clear:both;" />
          <div class="label" style="width:100%;"><span class='identifier'>web review / source</span><br /><a href="#{web_url}">#{web_url}</a></div>
          <br style="clear:both;" />
...
<body onload="showDivs(['button_location','button_genre']);">
<div id="content" >
  <div id="items">
     #{items_html}
  </div>
  ...
  if id == nil || id == '' then #new item
     ...
     subset_ids.each do | id |
       entry_form << item_form_template(
          ...
          $collection.items[id].pages,
          $collection.items[id].authors.encode("UTF-16BE", :invalid=>:replace, :replace=>"?").encode("UTF-8"),
          $collection.items[id].publication_date,
          $collection.items[id].disposition,
          $collection.items[id].language,
          $collection.items[id].edition
        )
     end #each subset_id
  end #else
      ...
      <form method='POST' action='/save_item' enctype="multipart/form-data">
        ...
        <div id="core" class="attributes" >
          <div class="item" style="padding-bottom:1px;float:left;">name*<br /><input name='title' id='title' type='text' size='50rem' value="#{title}" />
            ...
            <br style="clear:both;" />
            <div class="item" style="padding-bottom:1px;float:left;">author(s)<br /><input name='authors' id='authors' type='text' size='50rem' value="#{authors}" /></div>
            <br style="clear:both;" />
            <div class="item" style="padding-bottom:2px;">description<br /><textarea name='description' id='description' rows="8" cols="50rem">#{description}</textarea></div>
            <br style="clear:both;" />
        ...
        <div id="publishing_information" class="attributes" >
          <div class="item" style="float:left;padding-bottom:2px;">isbn<br /><input name='isbn' id='isbn' type='text' size="11rem" value='#{isbn}' /></div>
          <div class="item" style="float:left;padding-bottom:2px;">format<br /><input name='format' id='format' type='text' size="11rem" value='#{format}' /></div>
          <div class="item" style="float:left;padding-bottom:2px;">pages<br /><input name='pages' id='pages' type='text' size="11rem" value='#{pages}' /></div>
          <div class="item" style="float:left;padding-bottom:2px;">edition<br /><input name='edition' id='edition' type='text' size="15rem" value='#{edition}' /></div>
          <div class="item" style="float:left;padding-bottom:2px;">language<br /><input name='language' id='language' type='text' size="11rem" value='#{language}' /></div>
          <div class="item" style="float:left;padding-bottom:2px;">publisher<br /><input name='publisher' id='publisher' type='text' size="11rem" value='#{publisher}' /></div>
          <div class="item" style="float:left;padding-bottom:2px;">published<br /><input name='publication_date' id='publication_date' type='text' size="11rem" value='#{publication_date}' /></div>
        </div>
        ...
        <button type="button" class="collapsible" id="button_valuation" style="color:#8a8370;background-color:#c9c9b7;padding:1px;">&#8227; valuation</button>
        <div id="valuation" class="attributes">
          ...
          <div class="item" style="float:left;padding-bottom:2px;">disposition date<br /><input name='disposition' id='disposition' type='date' size="11rem" value='#{disposition}' /></div>
        ...
        <div id="links" class="attributes">
          ...
          <div class="item" style="padding-bottom:2px;">notes<br /><textarea name='notes' id='notes' rows="3" cols="60rem">#{notes}</textarea></div>
          <br style="clear:both;" /><hr />
        </div>
     </form>
     <a href="javascript:toggle('delete');">&#8227; delete</a><br />
...
class SaveItems < WEBrick::HTTPServlet::AbstractServlet
 ...
 def save_item(request)
    ...
    language     = request.query['language'].to_s.gsub(/[^\w\s\.\-,()]/,'')
    authors      = request.query['authors'].to_s.gsub(/[^\w\s\.\-,()]/,'')
    isbn         = request.query['isbn'].to_s.gsub(/[^0-9a-z]/i,'')
    disposition  = request.query['disposition'].to_s.gsub(/[^\w\s\.\-,()]/,'')
    pages        = request.query['pages'].to_s.gsub(/[\D]/,'')
    edition      = request.query['edition'].to_s.gsub(/[^\w\s\.\-,()]/,'')
...
##############################################################################
 ...
 #Navigational menus
 $MENUS     = ['location','genre','tags','format']
 $PICKLISTS = ['location','genre','tags','format']
